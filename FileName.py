...
def castVote():
    ...
    try:
        confirm = int(input("  Confirm your vote? (1 = Yes / 2 = No): "))
    except ValueError:
...
def showResults():
    ...
    for i in range(MAX_CANDIDATES):
        ...
        bars = int(percent / 4)
        print(f"{i + 1:<4}{candidateNames[i]:<18}{votes[i]:<8}{percent:5.1f}% " + "#" * bars)
...
def adminStartElection():
    ...
    if electionOpen:
        ...
        electionOpen = True
        print("  [OK] Election has been STARTED successfully.")
